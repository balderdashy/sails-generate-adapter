...
module.exports = function(scope, cb) {
	...
	if (scope.args) {
		if (scope.args[0]) {
			...
			scope.adaptersPath = path.join(DEFAULT_ADAPTERS_PATH, scope.args[0]);
	...
	if (!scope.adapterName) {
		return cb(new Error(
			'Missing argument: Please provide an `adapterType` for this adapter.\n' +
			'(should refer to the type of connection/database it makes, e.g. `mysql` or `irc`).'
		));
	}
	...
	_.defaults(scope, {
		...
		moduleName: 'waterline-' + ('' + scope.adapterName).toLowerCase(),
	});
